<div class="flex flex-col gap-4">
    ...
    {{-- ─────────────────────────────────────────────────────────────────────
         Page Header
         The "Export CSV" button requires "subscriptions.manage" because
         exporting is a write/privileged operation, not a simple view action.
    ─────────────────────────────────────────────────────────────────────── --}}
    <div class="flex items-center justify-between">
        <flux:heading size="lg">{{ __('Newsletter Subscribers') }}</flux:heading>

        {{-- Requires: subscriptions.manage --}}
    ...
    {{-- ─────────────────────────────────────────────────────────────────────
         Filters: email search + status dropdown
    ─────────────────────────────────────────────────────────────────────── --}}
            ...
            <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800 bg-white dark:bg-zinc-900">
                @forelse ($subscriptions as $sub)
                    <tr>
                        ...
                        {{-- Status badge (always visible, read-only) --}}
                        <td class="px-4 py-3">
                        ...
                        {{-- ─────────────────────────────────────────────────
                             Unsubscribe action
                             Requires: subscriptions.manage
                             Only shown when the subscription is currently
                             active; once unsubscribed the action disappears.
                        ───────────────────────────────────────────────────── --}}
                            ...
                            @if ($sub->status->value === 'active')
                                @can('subscriptions.manage')
                                    <flux:button
                                        ...
                                        wire:confirm="{{ __('Unsubscribe this email?') }}"
                                        size="sm"
                                        variant="ghost"
                                    >
                                        {{ __('Unsubscribe') }}
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-10 text-center text-zinc-400">{{ __('No subscribers found.') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
