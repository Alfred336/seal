        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800 bg-white dark:bg-zinc-900">
                @forelse ($requests as $request)
                    <tr>
                        {{-- Requester name + optional notes preview --}}
                        ...
                        {{-- ─────────────────────────────────────────────────
                             Status cell
                             Requires: call-requests.update
                             Users with this permission see an editable select
                             dropdown to change the request status.
                             All other users (view-only) see a read-only badge.
                        ───────────────────────────────────────────────────── --}}
                        <td class="px-4 py-3">
                            @can('call-requests.update')
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-4 py-10 text-center text-zinc-400">{{ __('No call requests found.') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
